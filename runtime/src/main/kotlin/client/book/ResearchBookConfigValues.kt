...
object ResearchBookConfigValues {
    fun spaceColor(panX: Float, panY: Float, zoom: Float): Int {
        ...
        val starDensity = if ((space.starDensity.takeIf(Float::isFinite) ?: 1F) < 0.875F) 0 else 1
        val starSize = if ((space.starSize.takeIf(Float::isFinite) ?: 1F) >= 1.25F) 1 else 0

        val lowY = encodedY and OFFSET_LOW_MASK
        val red = encodedX ushr OFFSET_LOW_BITS
        val green = encodedY ushr OFFSET_LOW_BITS
        val blue = ((lowY ushr 3) shl 7) or (starSize shl 6) or (starDensity shl 5) or encodedZoom
