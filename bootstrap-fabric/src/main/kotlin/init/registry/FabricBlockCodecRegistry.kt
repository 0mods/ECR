...
object FabricBlockCodecRegistry : BlockCodecRegistry {
    override val assembledMultiblockPart: MapCodec<AssembledMultiblockPartBlock> =
        register(
            ECRModIDs.ASSEMBLED_MULTIBLOCK_PART,
            BlockBehaviour.simpleCodec(::AssembledMultiblockPartBlock),
        )
    override val solarPrism: MapCodec<SolarPrism> =
        register(
            ECRModIDs.SOLAR_PRISM,
            BlockBehaviour.simpleCodec(::SolarPrism),
        )
    override val clusterBlock: MapCodec<ClusterBlock> =
        register(
            ECRModIDs.CLUSTER,
            BlockBehaviour.simpleCodec(::ClusterBlock),
        )
    override val crystalBlock: MapCodec<CrystalBlock> =
        register(
            ECRModIDs.CRYSTAL,
            BlockBehaviour.simpleCodec(::CrystalBlock),
        )
    override val mithrilineFurnace: MapCodec<MithrilineFurnace> =
        register(
    ...
    override val matrixDestructor: MapCodec<MatrixDestructor> =
        register(
            ECRModIDs.MATRIX_DESTRUCTOR,
            BlockBehaviour.simpleCodec(::MatrixDestructor),
        )
    override val coldDistiller: MapCodec<ColdDistiller> =
        register(
    ...
    override val enrichmentChamberExtractor: MapCodec<EnrichmentChamberExtractor> =
        register(
            ECRModIDs.ENRICHMENT_CHAMBER_EXTRACTOR,
            BlockBehaviour.simpleCodec(::EnrichmentChamberExtractor),
        )
    override val enrichmentChamberReceiver: MapCodec<EnrichmentChamberReceiver> =
        register(
            ECRModIDs.ENRICHMENT_CHAMBER_RECEIVER,
            BlockBehaviour.simpleCodec(::EnrichmentChamberReceiver),
        )
    override val rayTower: MapCodec<RayTower> =
        register(
            ECRModIDs.RAY_TOWER,
            BlockBehaviour.simpleCodec(::RayTower),
        )
    override val creativeMRUSource: MapCodec<CreativeMRUSource> =
        register(
