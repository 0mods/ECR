...
object BedrockParticleRenderTypes {
    private val additivePipeline = RenderPipelinesAccessor.ecrApiRegister(
        RenderPipeline.builder(RenderPipelinesAccessor.ecrApiParticleSnippet())
    ...
    internal fun get(texture: Identifier, material: BedrockParticleFile.Material): RenderType =
        cache.computeIfAbsent(Key(texture, material), ::create)

    private fun create(key: Key): RenderType {
        val pipeline = when (key.material) {
    ...
    private data class Key(val texture: Identifier, val material: BedrockParticleFile.Material)
}
